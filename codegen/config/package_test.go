...
func TestPackageConfig(t *testing.T) {
	t.Run("when given just a filename", func(t *testing.T) {
		...
		require.Equal(t, "github.com/sujamess/fastgql/codegen/config/testdata", p.ImportPath())

		require.Equal(t, "config_test_data", p.Pkg().Name())
		require.Equal(t, "github.com/sujamess/fastgql/codegen/config/testdata", p.Pkg().Path())
