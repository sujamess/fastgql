...
func TestImportPathForDir(t *testing.T) {
	...
	assert.Equal(t, "github.com/sujamess/fastgql/internal/code", ImportPathForDir(wd))
	assert.Equal(t, "github.com/sujamess/fastgql/api", ImportPathForDir(filepath.Join(wd, "..", "..", "api")))

	// doesnt contain go code, but should still give a valid import path
	assert.Equal(t, "github.com/sujamess/fastgql/docs", ImportPathForDir(filepath.Join(wd, "..", "..", "docs")))

	// directory does not exist
	assert.Equal(t, "github.com/sujamess/fastgql/dos", ImportPathForDir(filepath.Join(wd, "..", "..", "dos")))
